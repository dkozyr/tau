...
CodecsMap FilterH264Codec(const CodecsMap& origin, bool asymmetry_allowed) {
    ...
    for(auto& [pt, codec] : origin) {
        if(codec.name == "H264") {
            if(codec.format.find("packetization-mode=0") != etl::string_view::npos) {
                continue;
            }
            if(asymmetry_allowed) {
                if(codec.format.find("level-asymmetry-allowed=0") != etl::string_view::npos) {
                    continue;
                }
            }
            codecs.insert({pt, codec});
...
bool IsH264SupportAsymmetry(const CodecsMap& codecs) {
    for(auto& [_, codec] : codecs) {
        if(codec.name == "H264") {
            if(codec.format.find("level-asymmetry-allowed=1") != etl::string_view::npos) {
                return true;
            }
        }
    }
