...
TEST_F(H264Avc3NaluProcessorTest, Basic) {
    _avc3_nalu_processor->Push(CreateH264Nalu(kSps));
    _avc3_nalu_processor->Push(CreateH264Nalu(kPps));
    ASSERT_NO_FATAL_FAILURE(AssertProcessedNalUnits({kSps, kPps}));

    _avc3_nalu_processor->Push(CreateH264Nalu(kSei));
    _avc3_nalu_processor->Push(CreateH264Nalu(kIdr));
    ASSERT_NO_FATAL_FAILURE(AssertProcessedNalUnits({kSps, kPps, kIdr}));

    _avc3_nalu_processor->Push(CreateH264Nalu(kNonIdr));
    _avc3_nalu_processor->Push(CreateH264Nalu(kNonIdr));
