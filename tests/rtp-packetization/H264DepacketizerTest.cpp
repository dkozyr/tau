...
TEST_F(H264DepacketizerTest, AggregationLengthExceedsRemainingPayload) {
...
TEST_F(H264DepacketizerTest, SkipFuAWithoutEnd) {
    auto nalu1 = CreateH264Nalu(NaluType::kIdr, 2222);
    auto nalu2 = CreateH264Nalu(NaluType::kSei, 777);
...
TEST_F(H264DepacketizerTest, SkipFuAWithStartAndStop) {
    auto nalu = CreateH264Nalu(NaluType::kSei, 3333);
    ASSERT_TRUE(_ctx->packetizer.Process(nalu, true));
    ASSERT_EQ(3, _rtp_packets.size());

    _rtp_packets[0].GetView().ptr[kFixedHeaderSize + sizeof(FuAIndicator)] = CreateFuHeader(true, true, NaluType::kSei);
    ASSERT_FALSE(_ctx->depacketizer.Process(std::move(_rtp_packets)));
    ASSERT_EQ(0, _nal_units.size());
}
...
TEST_F(H264DepacketizerTest, SkipFuAWithInvalidMiddlePacket) {
    for(bool empty_payload : {true, false}) {
        SCOPED_TRACE(empty_payload ? "empty payload" : "forbidden bit");

        Init();
        auto unit = CreateH264Nalu(NaluType::kSei, 3333);
        ASSERT_TRUE(_ctx->packetizer.Process(unit, true));
        ASSERT_EQ(3, _rtp_packets.size());
        if(empty_payload) {
            _rtp_packets[1].SetSize(kFixedHeaderSize);
        } else {
            _rtp_packets[1].GetView().ptr[kFixedHeaderSize] |= 0b10000000;
        }
...
TEST_F(H264DepacketizerTest, SkipFuAWithWrongType) {
    ...
    _rtp_packets[1].GetView().ptr[kFixedHeaderSize + sizeof(FuAIndicator)] = CreateFuHeader(false, false, NaluType::kFuA);
    ASSERT_FALSE(_ctx->depacketizer.Process(std::move(_rtp_packets)));
    ASSERT_EQ(0, _nal_units.size());
}

TEST_F(H264DepacketizerTest, NaluForbiddenBit) {
    auto nalu = CreateH264Nalu(NaluType::kSei, 777);
    ASSERT_TRUE(_ctx->packetizer.Process(nalu, true));
