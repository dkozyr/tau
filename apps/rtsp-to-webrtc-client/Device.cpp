...
void Device::SendFrame(Buffer&& frame) {
    std::lock_guard lock{_frame_mutex};
    _frame.emplace(std::move(frame));
}
        ...
        webrtc::PeerConnection::Options{
            .sdp = {
                .audio = sdp::Media{
                    ...
                    .codecs = {
                        { 0, sdp::Codec{.index = 0, .name = "PCMU", .clock_rate = 8000}},
                    },
                    .ssrc = std::nullopt
                },
                .video = sdp::Media{
...
void Device::CreatePacketizer() {
    const auto& video = _pc->GetLocalSdp().medias[1];
    _video_ssrc = video.ssrc.value_or(0);
    auto pt = video.codecs.begin()->first;
    TAU_LOG_INFO("Video ssrc: " << _video_ssrc << ", pt: " << (size_t)pt);

    rtp::Writer::Options header_options = {
        .pt     = pt,
        .ssrc   = _video_ssrc,
        .ts     = _random.Int<uint32_t>(),
        .sn     = _random.Int<uint16_t>(),
        .marker = true
    };

    _rtp_allocator.emplace(_deps.udp_allocator,
        rtp::RtpAllocator::Options{
            .header = header_options,
            .base_tp = _deps.clock.Now(),
            .clock_rate = 90'000 //TODO: create global constant
        });
    _h264_packetizer.emplace(*_rtp_allocator);
    _h264_packetizer->SetCallback([this](Buffer&& packet) {
        if(_pc) {
            _pc->SendRtp(1, std::move(packet));
        }
    });
}
...
void Device::OnTimer() {
    if(_pc) {
        ...
        if(_state == webrtc::State::kConnected) {
            std::lock_guard lock{_frame_mutex};
            if(_frame) {
                const auto header = reinterpret_cast<const h264::NaluHeader*>(&_frame->GetView().ptr[0]);
                auto last = (header->type == h264::kIdr) || (header->type == h264::kNonIdr);
                _h264_packetizer->Process(std::move(*_frame), last);
            }
            _frame.reset();
        }
    }
